...
export default function Home() {
  ...
  const fetchActivities = async () => {
    try {
      ...
      const errorMessage = err && typeof err === 'object' && 'message' in err ? (err as { message?: string }).message || 'Aktiviteler alınamadı' : 'Aktiviteler alınamadı';
  ...
  return (
    <div className={`flex-1 bg-gradient-to-br min-h-screen p-6 ${theme === 'dark' ? 'from-slate-900 to-blue-950' : 'from-slate-50 to-blue-50'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        {statsLoading ? (
                      <div className="col-span-5 text-center text-gray-400">Yükleniyor...</div>
        ) : statsError ? (
          ...
          <>
            ...
            <div className={`rounded-lg shadow-sm border p-4 hover:shadow-lg hover:scale-105 transition-all duration-300 cursor-pointer ${theme === 'dark' ? 'bg-slate-800 border-slate-700 hover:bg-slate-700 hover:border-slate-600' : 'bg-white border-gray-200 hover:bg-gray-50 hover:border-gray-300'}`}> 
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={`text-2xl font-bold ${theme === 'dark' ? 'text-gray-100' : 'text-gray-800'}`}>{stats?.total_amount ? `₺${Number(stats.total_amount).toLocaleString('tr-TR')}` : '-'}</p>
                </div>
            ...
            > 
              <div className="flex items-center justify-between">
                <div>
                  <p className={`text-sm font-medium mb-1 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>Toplam Müşteri</p>
      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
          ...
          >
            ...
            <div className="text-sm font-semibold">Yeni İşlem</div>
          </motion.button>
          ...
          >
            <div className="text-2xl mb-2">🚗</div>
            <div className="text-sm font-semibold">Araç Ekle</div>
          </motion.button>
          ...
          >
            <div className="text-2xl mb-2">👥</div>
            <div className="text-sm font-semibold">Personel Ekle</div>
          </motion.button>
          ...
          >
            <div className="text-2xl mb-2">📊</div>
            <div className="text-sm font-semibold">İşlemler</div>
          </motion.button>
          ...
          >
            <div className="text-2xl mb-2">💰</div>
            <div className="text-sm font-semibold">Gelir Raporu</div>
          </motion.button>
          ...
          >
            <div className="text-2xl mb-2">👥</div>
            <div className="text-sm font-semibold">Müşteriler</div>
          </motion.button>
        </div>
      </motion.div>

      {/* Son Aktiviteler ve Hoş Geldiniz Mesajı - Yan Yana */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        >
          ...
          {activitiesLoading ? (
            <div className="text-center py-8">
              ...
              <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>Aktiviteler yükleniyor...</p>
            </div>
          ) : activitiesError ? (
            <div className="text-center py-8">
            ...
            <div className="text-center py-8">
              ...
              <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>Henüz aktivite bulunmuyor</p>
            </div>
          ) : (
        ...
        >
          <div className="text-center">
            ...
            <h2 className={`text-2xl font-bold mb-4 ${theme === 'dark' ? 'text-gray-100' : 'text-gray-800'}`}>
              Araç Filo Yönetim Sistemine Hoş Geldiniz!
            </h2>
            <p className={`text-lg mb-6 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
              Araçlarınızı, personelinizi ve işlemlerinizi kolayca yönetin. 
              Sol menüden istediğiniz bölüme erişebilirsiniz.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              <div className={`p-4 rounded-lg ${theme === 'dark' ? 'bg-slate-700' : 'bg-gray-50'}`}>
                <div className="text-2xl mb-2">🚗</div>
                <h3 className={`font-semibold mb-2 ${theme === 'dark' ? 'text-gray-100' : 'text-gray-800'}`}>Araç Yönetimi</h3>
                <p className={theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}>
                  Araç ekleyin, düzenleyin ve takip edin
                </p>
              </div>
              <div className={`p-4 rounded-lg ${theme === 'dark' ? 'bg-slate-700' : 'bg-gray-50'}`}>
                <div className="text-2xl mb-2">👥</div>
                <h3 className={`font-semibold mb-2 ${theme === 'dark' ? 'text-gray-100' : 'text-gray-800'}`}>Personel Yönetimi</h3>
                <p className={theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}>
              ...
              <div className={`p-4 rounded-lg ${theme === 'dark' ? 'bg-slate-700' : 'bg-gray-50'}`}>
                <div className="text-2xl mb-2">📊</div>
                <h3 className={`font-semibold mb-2 ${theme === 'dark' ? 'text-gray-100' : 'text-gray-800'}`}>İşlem Takibi</h3>
                <p className={theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}>
                  Gelir ve gider işlemlerini takip edin
                </p>
              </div>
            </div>
          </div>
